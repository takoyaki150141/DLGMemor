...
//  memui
//
//  Updated for iOS 16+ scene lifecycle.
//  - Looks up the active key window across connectedScenes
//  - Falls back to legacy UIApplication.keyWindow for pre-iOS-13 hosts
//  - Retries with exponential-ish backoff if the scene hasn't activated yet
//
...
+ (void)addDLGMemUIView:(id<DLGMemUIViewDelegate>)delegate {
    ...
    // No foreground window yet — listen for one to appear, then
    // install.  Bound to 30s so we don't leak a timer forever if the
    // scene never becomes active.
